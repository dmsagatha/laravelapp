<x-app-layout>
    ...
    <div class="flex float-right px-0 py-3 space-x-0 text-slate-800">
      @if ($view == 'index')
        ...
        <div id="actionButtons" class="hidden space-x-2 mt-2">
          <button type="button" id="deleteButton" data-action="/usuarios/eliminar-en-masa" data-method="DELETE" data-title="Confirmar Eliminación" data-message="¿Está seguro de que desea eliminar los registros seleccionados?" class="flex bg-red-600 text-white px-4 py-2 rounded hover:bg-red-500">
      ...
      @else
        ...
        <div id="actionButtons" class="hidden space-x-2 mt-4">
          ...
          <button
            data-action="/usuarios/forzar-eliminar"
            data-method="DELETE"
  ...
  <div id="actionModal" class="hidden fixed inset-0 z-10 justify-center items-center h-full w-full bg-black/50 transition-opacity duration-300 ease-in-out" role="dialog" aria-labelledby="modalTitle" aria-describedby="modalDescription">
    <!-- Modal content -->
    <div class="bg-slate-50 dark:bg-slate-700 rounded-lg shadow-lg p-4 max-w-lg w-full transform scale-95 transition-all duration-300 ease-in-out">
      ...
      <div class="flex justify-end space-x-2 my-4">
        <form id="modalForm" method="POST">
          ...
          <button id="confirmButton" type="submit" class="bg-red-600 border border-transparent rounded-md font-medium px-2 py-2 mr-2 text-center text-slate-50 hover:bg-red-500 focus:outline-none focus:border-red-700 focus:ring-0 focus:ring-red-200 active:bg-red-600 disabled:opacity-25 transition">
            Confirmar
          </button>
          <button id="modalCancelButton" type="button" class="bg-blue-600 border border-transparent rounded-md font-medium px-2 py-2 mr-2 text-center text-slate-50 hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:ring-0 focus:ring-blue-200 active:bg-blue-600 disabled:opacity-25 transition">
            Cancelar
          </button>
        </form>
      </div>
